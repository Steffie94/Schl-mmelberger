...
public class WorldGeneratorSkript : MonoBehaviour {
    ...
    private Vector3 currentPos; // Spawnort vor dem randomisieren
    private Vector3 worldObjectsStartPos; 
    private Vector3 terrainStartPos;

    private float groundWidth; // Variable zum Speichern der Bodenbreite

    private float worldObjectIncAmt; // Variable für die Distanz der WorldObjecte (Wie weit auseinander die Weltobjekte spawnen können)
    private float terrainIncAmt; // Variable für die Distanz der Terrain Objecte (Wie weit auseinander die Terrainobjekte spawnen können)

    private float worldObjectRandAmt; //Variable um zu entscheiden, wie weit die Weltobjekte auseinander liegen können
    private float terrainRandAmt; //Variable um zu entscheiden, wie weit die Terrainobjekte auseinander liegen können

    //Die Werte für die Spawn-Schleife durch das Gitter
    [SerializeField]
    private int worldObjectRowsAndCols; // Variable die sagt wie Nah die Welt Objekte spawnen
    [SerializeField]
    private int terrainRowsAndCols; //// Variable die sagt wie Nah die Terrain Objekte spawnen

    [SerializeField]
    private int repeatPasses;// zum wiederholen der Durchläufe
    private int currentPass; // um den aktuellen Durchlauf zu speichern 

    //Variablen zum überprüfen ob sich Objekte überschneiden 
    [SerializeField]
    ...
    {
        groundWidth = trMarker.transform.position.x - blMarker.transform.position.x; // Die Breitenbreite ist der Abstand unser Markers

        worldObjectIncAmt = groundWidth / worldObjectRowsAndCols; //Distanz
        worldObjectRandAmt = worldObjectIncAmt / 2f; // wie weit Objekte auseinander liegen 

        terrainIncAmt = groundWidth / terrainRowsAndCols; //Distanz
        terrainRandAmt = terrainIncAmt / 2f;  // wie weit Objekte auseinander liegen 
